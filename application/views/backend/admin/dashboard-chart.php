...
for ($i = 0; $i < 12; $i++) {
    ...
    $total_admin_revenue = $this->db->get('payment')->result();

    $total = array();
    foreach ($total_admin_revenue as $key => $getAdminRevenue) {

        if ($getAdminRevenue->course_type !== "course") {
            $revenue_percentage = $getAdminRevenue->revenue_percentage;
            $admin_percentage = 100 - $revenue_percentage;
            $adminAmount = ($getAdminRevenue->amount * $admin_percentage) / 100;

        } else {
            $adminAmount = $getAdminRevenue->admin_revenue;
        }

        array_push($total, $adminAmount);
    }
    array_push($month_wise_income, array_sum($total));
}
