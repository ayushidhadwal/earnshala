...
class Crud_model extends CI_Model
{
    ...
    public function get_revenue_by_user_type($timestamp_start = "", $timestamp_end = "", $revenue_type = "")
    {
        ...
        if ($revenue_type == 'admin_revenue') {
            ...
            $this->db->where('course_type !=', 'course');
        }
        $this->db->where('type_of_pay !=', 'partial_course');

        return $this->db->get('payment')->result_array();
    }

    public function get_revenue_by_user_type_partitial($timestamp_start = "", $timestamp_end = "", $revenue_type = "")
    {
    ...
    {
        if ($user_id > 0) {
            return $this->db->order_by('id','DESC')->get_where('payment', array('user_id' => $user_id));
        } else {
    ...
    public function add_course($param1 = "")
    {
        ...
        $data['short_description'] = strip_tags($this->input->post('short_description'));
        ...
        $data['is_upcoming_course'] = $this->input->post('is_upcoming_course') ? $this->input->post('is_upcoming_course') : '0';
        ...
        if (array_key_exists('system_video_file', $_FILES)) {
            if ($_FILES['system_video_file']['error'] !== UPLOAD_ERR_OK) {
                ...
                redirect(site_url(strtolower($this->session->userdata('role')) . '/course_form/add_course/'), 'refresh');
            }
        } else {
        ...
        if (!file_exists('uploads/courses_files/videos')) {
            mkdir('uploads/courses_files/videos', 0777, true);
        }
        $video_file_path = 'uploads/courses_files/videos/' . $uploadable_video_file;
        ...
        $this->db->where('id', $course_id)->update('course', ['thumbnail' => $data['thumbnail']]);

        if ($data['status'] == 'approved') {
            $this->session->set_flashdata('flash_message', get_phrase('course_added_successfully'));
            // $this->session->set_flashdata('course_add', "You can now add lessons to your courses");
            ...
            $this->session->set_flashdata('course_add', "You can now add lessons to your course");
    ...
    public function update_course($course_id, $type = "")
    {
        ...
        if ($this->input->post('is_upcoming_course') != 1) {
            $data['is_upcoming_course'] = '0';
        } else {
            $data['is_upcoming_course'] = '1';
        }
        ...
        if (isset($_FILES['system_video_file']) && !empty($_FILES['system_video_file']['name'])) {
            ...
            $previews_video_url = $this->db->get_where('course', array('id' => $course_id))->row('video_url_for_mobile_application');
            $video_file = explode('/', $previews_video_url);
            if (file_exists('uploads/courses_files/videos/' . end($video_file))) {
                unlink('uploads/courses_files/videos/' . end($video_file));
            }

//            echo 'uploads/courses_files/videos/'.end($video_file);
//            exit;
            ...
            if (!file_exists('uploads/courses/videos')) {
                mkdir('uploads/courses/videos', 0777, true);
            }
            $video_file_path = 'uploads/courses/videos/' . $uploadable_video_file;
            move_uploaded_file($tmp_video_file, $video_file_path);
    ...
    public function get_total_duration_of_lesson_by_course_id($course_id)
    {
        ...
        return sprintf("%02d", $hours) . ' ' . get_phrase('hours');
    }
    ...
    // Adding quiz functionalities
    public function add_quiz_new()
    {
        // $data['course_id'] = $course_id;
        $data['q_title'] = html_escape($this->input->post('title'));
        // $data['section_id'] = html_escape($this->input->post('section_id'));

        // $data['lesson_type'] = 'quiz';
        $data['q_duration'] = '00:00:00';
        $data['q_date_added'] = strtotime(date('D, d-M-Y'));
        $data['q_summary'] = html_escape($this->input->post('summary'));
    ...
    function add_multiple_choice_question($quiz_id)
    {
        ...
        if (sizeof($this->input->post('correct_answers')) == 0) {
            ...
            $correct_answers = $this->input->post('correct_answers')[0];
        }
    ...
    // updating quiz functionalities
    public function edit_quiz_new($quiz_id = "")
    {
        $data['q_title'] = html_escape($this->input->post('title'));
        // $data['section_id'] = html_escape($this->input->post('section_id'));
        $data['q_date_added'] = strtotime(date('D, d-M-Y'));
        $data['q_summary'] = html_escape($this->input->post('summary'));
        $this->db->where('q_id', $quiz_id);
    ...
    public function update_forum($forum_id, $status)
    {
        $this->db->where('f_id', $forum_id);
        $this->db->update('forum', ['f_status' => ($status == 1) ? 0 : 1]);
        return true;
    }
    ...
    public function forum_reply_on_reply($forum_id)
    {
        // $data = [
    ...
    public function get_forum_replies($forum_id)
    {
        $condition = [
            'fr_forum_id' => $forum_id,
            'fr_user_id' => $this->session->userdata('user_id')
        ];
        return $this->db->get_where('forum_reply', $condition);
    }

    public function get_forum_reply($forum_reply_id)
    {
        $condition = [
            'fr_id' => $forum_reply_id,
            'fr_user_id' => $this->session->userdata('user_id')
        ];
        return $this->db->get_where('forum_reply', $condition);
    }
    ...
    //Forum Reply
    public function delete_forum_reply($forum_reply_id)
    {
    ...
    public function update_forum_reply($forum_reply_id, $forum_id)
    {
        $data = [
            'fr_reply' => $this->input->post('reply')
        ];
        ...
        $this->db->update('forum_reply', $data);
        return true;
    }
    ...
    public function add_live_class()
    {
        ...
        if (!file_exists('uploads/thumbnails/liveclass')) {
            mkdir('uploads/thumbnails/liveclass', 0777, true);
        }
        $image = md5(rand(10000000, 20000000)) . '.jpg';
        move_uploaded_file($_FILES['image']['tmp_name'], 'uploads/thumbnails/liveclass/' . $image);

        $this->db->set('live_class_name', $this->input->post('class_name'));
        $this->db->set('description', $this->input->post('description'));
        $this->db->set('instructor_id', $this->input->post('instructor'));
        $this->db->set('live_payment_type', $this->input->post('payment_type'));
        $this->db->set('price', $this->input->post('price'));
        $this->db->set('live_date', $this->input->post('date'));
        $this->db->set('number_of_students', $this->input->post('number_of_students'));
        $this->db->set('live_start_time', $this->input->post('start_time'));
        $this->db->set('live_end_time', $this->input->post('end_time'));
        $this->db->set('live_duration', $this->input->post('duration'));
        $this->db->set('live_lang', $this->input->post('language'));
        $this->db->set('live_image', 'uploads/thumbnails/liveclass/' . $image);

        $this->db->insert('live_class_time_new');
    ...
    public function edit_live_class_new($live_id)
    {
        $this->db->set('live_class_name', $this->input->post('class_name'));
        $this->db->set('description', $this->input->post('description'));
        $this->db->set('instructor_id', $this->input->post('instructor'));
        ...
        $this->db->update('live_class_time_new');
    ...
    public function fetchLiveClassList($select2 = '', array $condition = [])
    {
        ...
        if (!empty($condition['sc_user_id'])) {
            $this->db->join('subscription_class sc', 'sc.sc_live_id=lctn.live_id', 'inner');
        }
        $this->db->where($condition);
        $this->db->order_by('live_date DESC, live_start_time DESC');
        return $this->db->get();
    }

    public function delete_live_class($live_class_id)
    {
        $this->db->where('live_id', $live_class_id);
        $this->db->delete('live_class_time_new');
        if ($this->db->affected_rows() > 0) {
            return true;
        } else {
            return false;
    ...
    // COUNSELLING SESSION CLASS
    public function add_counselling_session()
    {
        $this->db->set('cs_name', $this->input->post('counselling_name'));
        $this->db->set('description', $this->input->post('description'));
        $this->db->set('instructor_id', $this->input->post('instructor'));
        $this->db->set('cs_payment_type', $this->input->post('payment_type'));
        $this->db->set('price', $this->input->post('price'));
        $this->db->set('cs_date', $this->input->post('date'));
        $this->db->set('cs_start_time', $this->input->post('start_time'));
        $this->db->set('cs_end_time', $this->input->post('end_time'));


        $this->db->insert('counselling_session');
        if ($this->db->affected_rows() > 0) {
            return true;
        } else {
            return false;
    ...
    public function edit_counselling_session_new($cs_id)
    {
        $this->db->set('cs_name', $this->input->post('counselling_name'));
        $this->db->set('description', $this->input->post('description'));
        $this->db->set('instructor_id', $this->input->post('instructor'));
        ...
        $this->db->update('counselling_session');
    ...
    public function fetchCounsellingSessionList($select2 = '', array $condition = [])
    {
        ...
        // $this->db->where('instructor_id',$this->session->userdata('user_id'));
        if (!empty($condition['sc_user_id'])) {
            $this->db->join('subscription_counselling_session scc', 'scc.sc_cs_id=cs.cs_id', 'inner');
        }
        $this->db->where($condition);
        $this->db->order_by('cs_date DESC, cs_start_time DESC');
        return $this->db->get();
    }

    public function delete_counselling_session($cs_id)
    {
        $this->db->where('cs_id', $cs_id);
        $this->db->delete('counselling_session');
        if ($this->db->affected_rows() > 0) {
            return true;
        } else {
            return false;
    ...
    public function get_replies_on_reply_id($table, array $condition = [])
    {
        ...
        $this->db->select(['forum_reply.fr_id', 'forum_reply.fr_user_id as user_id', 'forum_reply.fr_forum_id', 'forum_reply.fr_reply', 'forum_reply.fr_created_date', 'users.first_name as user_name', 'users.image as user_image']);
        $this->db->from($table);
        $this->db->join('users', 'users.id = forum_reply.fr_user_id', 'right');
        $this->db->where($condition);
        // $this->db->where('fr_id',$forum_reply_id);
    ...
    public function enroll_live_student($liveClassId)
    {

        $this->db->from('enroll_live');
        $this->db->join('users', 'users.id = enroll_live.el_user_id');
        $this->db->where('el_live_id', $liveClassId);
        return $this->db->get();
    }

    public function enroll_counselling_session($select2 = '', array $condition = [])
    {
        $this->db->select($select2);
        $this->db->from('enroll_counselling');
        $this->db->join('users', 'users.id = enroll_counselling.ec_user_id');
        $this->db->where($condition);
        $this->db->order_by('enroll_counselling.ec_date DESC, enroll_counselling.ec_time DESC');
        return $this->db->get();
    }

    public function enroll_live_classes_by_user_id($user_id = "")
    {
        return $this->db->get_where('enroll_live', array('el_user_id' => $user_id));
    }

    public function enrol_counselling_session_by_user_id($user_id = "")
    {
        return $this->db->get_where('enroll_counselling', array('ec_user_id' => $user_id));
    }

    public function get_live_class_by_id($liveId = "")
    {
        return $this->db->get_where('live_class_time_new', array('live_id' => $liveId));
    }

    public function get_counselling_session_by_id($course_id = "")
    {
        return $this->db->get_where('counselling_session', array('id' => $course_id));
    }

    // GET PENDING PAYOUTS BY DATE RANGE

    public function get_pending_counselling_payouts($month, $year,$type)
    {
    ...
    public function get_completed_counselling_payouts($month = "", $year = "", $type)
    {
        $this->db->select('instructor_id');
        $this->db->from('payment');
        $this->db->group_by('instructor_id');
    ...
    public function adminSettlement()
    {

        $conditions = [
            'month' => $this->input->post('month'),
            'year' => $this->input->post('year'),
            'instructor_id' => $this->input->post('teacherId'),
    ...
    public function get_pending_live_payouts($month, $year)
    {

        // $this->db->select('payment.course_id, live_class_time_new.live_id, live_class_time_new.instructor_id, payment_type');
        // $this->db->join('live_class_time_new','live_class_time_new.live_id = payment.course_id','inner');
        // $this->db->from('payment');
        // $this->db->group_by('course_id');
        // $this->db->where('course_type', 'live');
        // $this->db->where('month', $month);
        // $this->db->where('year', $year);
        // $this->db->where('isPaid', 0);
        // return $this->db->get()->result_array();
    ...
    public function fetch_courses()
    {
        ...
        return $this->db->where(['status'=>'active','is_free_course'=> 0])->get('course')->result_array();
    }

    public function addBanner()
    {
        $data['banner_link'] = html_escape($this->input->post('link'));

        if (!file_exists('uploads/banners')) {
            mkdir('uploads/banners', 0777, true);
        }
        $banner_image = md5(rand(10000000, 20000000)) . '.jpg';
        move_uploaded_file($_FILES['file']['tmp_name'], 'uploads/banners/' . $banner_image);

        $data['banner_image'] = 'uploads/banners/' . $banner_image;
    ...
    {
        return $this->db
            ->select('course.title as title, banners.*')
            ->join('course', 'course.id = banners.banner_link', 'inner')
            ->get('banners')->result();
    }
    ...
    public function getCourseDetails($courseId){
        ...
        return $this->db->where('id',$courseId)->get('course')->row();
    }
    ...
    public function updatePayment($razorpayPaymentId,$razorpayPaymentLinkId,$razorpaySignature){

        $this->db->where('transaction_id',$razorpayPaymentLinkId);
        $this->db->update('payment',['razor_payment_id'=>$razorpayPaymentId,'razor_signature'=>$razorpaySignature,'isPaid'=>1]);
        if($this->db->affected_rows()>0){
            return true;
        }
        return true;
    ...
    public function getCoursePurchaseDetails($user_id,$courseId)
    {

        $this->db->select('payment.*,enrol.date_added as courseAddDate,enrol.date_expire as courseExpiredDate');
        $this->db->from('payment');
        $this->db->join('enrol','enrol.user_id=payment.user_id','inner');
        $this->db->order_by('payment.id','DESC');
        return $this->db->where(array('payment.user_id' => $user_id,'payment.course_id' => $courseId))->get()->row();
    }
    ...
    public function insert_coupons(){
        $this->db->set('cpn_name', $this->input->post('coupon_name'));
        $this->db->set('cpn_percent', $this->input->post('coupon_discount_percentage'));
        $this->db->set('cpn_max', $this->input->post('coupon_max_discount'));
        $this->db->set('short_description', $this->input->post('short_description'));
        $this->db->set('cpn_start_date', date('Y-m-d', strtotime($this->input->post('satrt_date'))));
        $this->db->set('cpn_end_date', date('Y-m-d', strtotime($this->input->post('end_date'))));
        if (!empty($this->input->post('no_of_times'))) {
            $n_times = $this->input->post('no_of_times');
        }else{
            $n_times = 0;
        }
        $this->db->set('cpn_no_of_times_used', $n_times);
        if (!empty($this->input->post('no_of_times_single_user'))) {
            $n_times_single_user = $this->input->post('no_of_times_single_user');
        }else{
            $n_times_single_user = 0;
        }
        $this->db->set('single_user_limit', $n_times_single_user);
        if (!empty($this->input->post('users'))) {
            $users = json_encode($this->input->post('users'));
        }else{
            $users = 'all';
        }
    ...
    public function delete_coupons($coupon_id){
    ...
    public function insert_cart(){
        foreach ($this->input->post('courses') as $key => $value) {
            $this->db->where('course_id', $value);
            $this->db->where('user_id', $this->input->post('user_id'));
            $check = $this->db->get('cart');
            if ($check->num_rows() > 0) {
                $this->db->where('course_id', $value);
                $this->db->where('user_id', $this->input->post('user_id'));
                $this->db->delete('cart');
            }
            $this->db->set('course_id', $value);
